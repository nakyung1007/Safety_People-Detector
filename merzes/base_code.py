        ...
        if self.enable_yolo and yolo_model_path:
            try:
                ...
                if os.path.exists(yolo_model_path):
                    ...
                    print(f"[CAM] YOLO 모델 로드 성공: {yolo_model_path}")
                else:
                    print(f"[CAM] YOLO 가중치 파일 없음: {yolo_model_path}")
...
class MyUI(QWidget):
    ...
    # MyUI 클래스 내부에 추가
    def execute_auto_tracking(self, person_centroids):
        # 시스템이 START(Green) 상태일 때만 동작
        if not self.state_icon.system_started or not person_centroids:
            return

        # 1. 가장 가까운 사람 선택 (첫 번째 centroid)
        cx, cy = person_centroids[0]

        # 2. 각도 계산
        pan, tilt = self.calculate_tracking_angles(cx, cy)

        # 3. 모터 명령 전송 (정수로 변환하여 전송)
        # s1: Pan, s2: Tilt
        self.sendCmd(f"s1:{int(pan)}")
        self.sendCmd(f"s2:{int(tilt)}")
        
        print(f"[AUTO TRACK] Target: ({cx:.2f}, {cy:.2f}) -> Pan: {pan}°, Tilt: {tilt}°")

    # main.py의 MyUI 클래스 내부에 추가하세요
    def calculate_tracking_angles(self, lx_m, ly_m):
        """
        라이다 좌표(m)를 받아 카메라 Pan/Tilt 각도(deg)로 변환
        조나경 님의 angle_calculation.py 로직 이식
        """
        # m 단위를 cm로 변환
        lx, ly = lx_m * 100.0, ly_m * 100.0
        lz = 0 

        # --- [실측 데이터 오프셋] ---
        off1 = {'x': 0, 'y': 35.7, 'z': 5.9}      # 라이다 ↔ Pan모터
        off2 = {'x': 0.8, 'y': 36.7, 'z': 0.9}    # Pan모터 ↔ Tilt모터
        off3 = {'x': 2, 'y': 0, 'z': 5.4}         # Tilt모터 ↔ 카메라 렌즈

        # --- [좌표 변환 및 보정] ---
        m1_x, m1_y, m1_z = lx + off1['x'], ly + off1['y'], lz + off1['z']
        m2_x, m2_y, m2_z = m1_x + off2['x'], m1_y + off2['y'], m1_z + off2['z']
        ...
        # --- [최종 각도 계산] ---
        pan_deg = math.degrees(math.atan2(m2_x, m2_y))
        
        horizontal_dist = math.sqrt(target_x**2 + target_y**2)
        tilt_deg = math.degrees(math.atan2(target_z, horizontal_dist))
    ...
    def check_device_status(self):
        ...
        # 카메라 상태 확인
        camera_alive = (now - self.last_camera_frame_time < 5.0)
